...
public final class EntitiesModPlugin extends JavaPlugin {
  ...
  private final EntitiesModTemplate service = new EntitiesModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-entities-mod-worker");
  ...
  public EntitiesModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new EntitiesModStatusCommand());
    getCommandRegistry().registerCommand(new EntitiesModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[EntitiesMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[EntitiesMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class EntitiesModStatusCommand extends CommandBase {
  ...
  private final class EntitiesModDemoCommand extends CommandBase {
